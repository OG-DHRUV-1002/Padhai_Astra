...
export default function AdminNotificationsPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div>
        <h1 className="text-3xl font-bold text-white mb-2">Campus Administration Alerts</h1>
        <p className="text-gray-400">
          {unreadCount > 0
            ? `${unreadCount} unread system alert${unreadCount === 1 ? "" : "s"}`
            : "System events, issues reported, and facility notifications"}
        </p>
      </div>

      {isLoading && notifications.length === 0 && (
        <div className="p-12 text-center text-sm text-gray-400 flex items-center justify-center gap-2">
          <Sparkles className="w-5 h-5 animate-spin text-red-500" /> Loading system notifications...
        </div>
      )}
