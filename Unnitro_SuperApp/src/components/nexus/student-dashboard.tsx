...
export default function StudentDashboard() {
  ...
  const [aiTip, setAiTip] = useState({ tip: "Active recall > passive reading. Test yourself instead of re-reading.", emoji: "dY'" });
  const [aiLevel, setAiLevel] = useState(12);
  const [aiFocus, setAiFocus] = useState("Deep Work");
  ...
  const dashboardBentoItems: BentoItem[] = [
    // 0. Archi AI Tip — Bento Item (colSpan: 1)
    {
      title: "Archi's Tip of the Day",
      meta: aiTip.emoji,
      description: aiTip.tip,
        ...
        <div ref={headerRef} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <h1 className="text-3xl font-bold text-white tracking-tight">Welcome back, {studentData?.name?.split(' ')[0] || "Arjun"}</h1>
              ...
              <Badge variant="secondary" className="px-2.5 py-1 text-[10px] bg-white/5 border-white/10 text-white backdrop-blur-md font-bold uppercase">
                <Sparkles className="h-3 w-3 mr-1 text-yellow-400" />Lvl {aiLevel}
              </Badge>
              <Badge variant="secondary" className="px-2.5 py-1 text-[10px] bg-white/5 border-white/10 text-white backdrop-blur-md font-bold uppercase">
                <Zap className="h-3 w-3 mr-1 text-blue-400" />{aiFocus}
              </Badge>
            </div>
